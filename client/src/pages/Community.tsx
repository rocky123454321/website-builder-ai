  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        ...
        console.log('error in fetchProjects')
      }
    }
    loadProjects()
  }, [fetchProjects])
                 ...
                 className='w-72 max-sm:mx-auto cursor-pointer
                  ...
                  <div className='relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800'>
                    {project.current_code ? (
                      <iframe srcDoc={project.current_code} className='absolute top-0 left-0 w-300 h-200 origin-top-left pointer-events-none' style={{ transform: 'scale(0.25)' }} />
                    ) : (
                  ...
                  <div className='p-4 text-white bg-linear-180 from-transparent
                   group-hover:from-indigo-950 to-transparent
                   transition-colors'>

                    <div className='flex items-start justify-between'>
                      <h2 className='text-lg font-medium line-clamp-2'>{project.name}</h2>
                      <button className='px-2.5 py-0.5 mt-0.5  ml-2 text-xs bg-gray-800 border border-gray-700 rounded-full'>
                        Website</button>
                    ...
                    <div  className='flex justify-between items-center mt-6'>
                      ...
                      <div className='flex gap-3 text-white text-sm'>
                        <button  className='px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md  
                        transition-colors flex items-center gap-2'><span>{project.user?.name?.slice(0,1)}</span>
                        {project.user?.name}
                        </button>
